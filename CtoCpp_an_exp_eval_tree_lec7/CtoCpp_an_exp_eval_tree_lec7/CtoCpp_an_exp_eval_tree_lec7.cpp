// CtoCpp_an_exp_eval_tree_lec7.cpp : 콘솔 응용 프로그램에 대한 진입점을 정의합니다.
//

#include "stdafx.h"

#include <iostream>
#include <memory>
#include <string>

using std::ostream;
using std::cout;
using std::cerr;
using std::endl;

int valtab[127];	//	integer values of variables

class Tree;

class Node{		//	abstract base class
private:
	friend class Tree;		//	friend means kissing cousins. O.k. to see their private properties. 
	friend ostream& operator<<(ostream&, const Tree&);
	int use;	//	reference count
protected:
	virtual void print(ostream& o) = 0;	//	'const' means member variables cannot be mutated in the function. '= 0' means it's pure virtual(they are to be overwritten), no more meaning.
	virtual int eval() = 0;
	virtual ~Node(){}	//	virtual destructor
};

class Tree{
private:
	friend class Node;
	friend ostream& operator<<(ostream&, const Tree&);
	Node* p;	//	polymorphic hierarchy, 'p' can point whatever classes which inherits Node class.
public:
	Tree(int);	//	constant
	Tree(const char);	//	variable
	Tree(const char, Tree);	//	unary operator
	Tree(const char, Tree, Tree);	//	binary operator
	Tree(const Tree& t)		//	copy constructor
	{
		p = t.p;
		++p->use;
	}
	~Tree()
	{
		if(--p->use == 0){
			delete p;
		}
	}
	void operator=(const Tree& t);
	int eval()
	{
		return p->eval();
	}
};

void Tree::operator=(const Tree& t)
{
	++t.p->use;
	if(--p->use == 0){
		delete p;
	}
	p = t.p;
}

ostream& operator<<(ostream& o, const Tree& t)
{
	t.p->print(o);	//	polymorphic print. pointer that selects appropriate virtual function according to 'o'
	return(o);
}

class Leafnode:public Node
{
private:
	friend class Tree;
	virtual void print(ostream& o) = 0;
	virtual int eval() = 0;
};

class IntNode:public Leafnode
{
private:
	friend class Tree;
	int n;
	void print(ostream& o)
	{
		o << n;
	}
	IntNode(int k): n(k) {}
public:
	int eval()
	{
		return n;
	}
};

class IdNode:public Leafnode
{
private:
	friend class Tree;
	char name;
	void print(ostream& o)
	{
		o << name;
	}
	IdNode(const char id): name(id) {}
public:
	int eval()
	{
		return valtab[name];
	}
};

class UnaryNode:public Node
{
private:
	friend class Tree;
	char op;
	Tree opnd;
	UnaryNode(const char a, Tree b): op(a), opnd(b) {}
	void print(ostream& o)
	{
		o << "(" << op << opnd << ")";
	}
public:
	int eval()
	{
		switch(op){
		case '-': return(-opnd.eval());
		case '+': return(+opnd.eval());
		default: cerr << "no operand\n" << endl;
		}

		return 0;
	}
};

class BinaryNode:public Node
{
private:
	friend class Tree;
	char op;
	Tree left;
	Tree right;
	BinaryNode(const char a, Tree b, Tree c): op(a), left(b), right(c) {}
	void print(ostream& o) {o << "(" << left << op << right << ")";}
public:
	int eval()
	{
		switch(op){
		case '-': return (left.eval() - right.eval());
		case '+': return (left.eval() + right.eval());
		case '*': return (left.eval() * right.eval());
		default: cerr << "no operation\n" << endl;
		}

		return 0;
	}
};

Tree::Tree(int n)
{
	p = new IntNode(n);
}

Tree::Tree(const char id)
{
	p = new IdNode(id);
}

Tree::Tree(const char op, Tree t)
{
	p = new UnaryNode(op, t);
}

Tree::Tree(const char op, Tree left, Tree right)
{
	p = new BinaryNode(op, left, right);
}

int _tmain(int argc, _TCHAR* argv[])
{
	valtab['A'] = 3;
	valtab['B'] = 4;
	Tree t1 = Tree('*', Tree('-', 5), Tree('+', 'A', 4));
	Tree t2 = Tree('+', Tree('-', 'A', 1), Tree('+', t1, 'B'));
	cout << "t1 = " << t1 << " ; t2 = " << t2 << endl;
	cout << "t1:" << t1.eval() << " t2:" << t2.eval() << endl;

	return 0;
}

